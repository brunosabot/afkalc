...
const ChooseHero: React.FC<Props> = ({ current:[currentId, si, inn, currentArtifact], onSelect }) => {
  const { t } = useTranslation("common");

  return (
    <div className={styles.ChooseHero}>
      <div className={styles.InputWrapper}>
        <InputField style={{width: "60px"}} small name="si" label={t(`concept.si`)} value={si} onChange={(value) => onSelect(DetailType.SI, parseInt(value, 10))} />
        <InputField style={{width: "60px"}} small name="inn" label={t(`concept.inn`)} value={inn} onChange={(value) => onSelect(DetailType.INN, parseInt(value, 10))} />
      </div>
